...
컷다가 작았다가 컷다가 작았다가 하는 것의 길이가 얼마나 가장 긴가 하는 문제
큰지 작은지 하는 플래그하고, 전의 값이 나보다 큰 지 작은지 함께 비교하면 될 것 같은데
...
함수 위글맥스길이(넘스)
    ...
    길이, 업 = 1, 넌
    (1, 길이) 만큼 반복
        내 값이 내 전의 값보다 크고, (업이 넌 이거나 업이 폴스면)
            길이++
            업 = 트루
        내 값이 내 전의 값보다 작고, (업이 넌이거나 업이 트루면)
            길이++
            업 = 폴스
